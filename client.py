...
class Receive(threading.Thread):
    ...
    Attributes:
        sock (socket.socket): The connected socket ovject
        name (str): The username provided by the user
        message (tk.Listbox): The tk.Listbox object containing all messages displayed on the GUI
    """
    ...
    def run(self):
        """
        Receives data from the server and displays it in the GUU
...
class Client:
    """
    Supports management of client-server connections and integratin with the GUI
    Attributes:
        ...
        messsages (tk.Listbox): The tk.Listbox object containing all messages displayed on the GUI
    """
...
def main(host, port):
    ...
    frm_messages.grid(row=0, column=0, columnspan=2, stick="nsew")
    ...
    frm_entry.grid(row=1, column=0, padx=10, stick="ew")
    btn_send.grid(row=1, column=1, pady=10, stick="ew")
